...
const int num_obj = 7;
int main() {
	Area *list[num_obj];						// Creating an array of objects
	...
	list[0] = &blue_ring;						// pointing the list[0] the blue ring object/instance in the first block of the array[0]
	...
	while (index < num_obj) {					// condition for iterating over the list, we only have 7 elemetns so it will stop at index=7
		(list[index])->getColor();				
		double area = list[index++]->calcArea();// taking the values of area form every shape and storing it into area and calculating area 
		sum_area += area;
